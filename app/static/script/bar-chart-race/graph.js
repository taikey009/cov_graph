...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      'patients.csv',
      // new URL('./files/aec3792837253d4c6168f9bbecdf495140a5f9bb1cdb12c7c8113cec26332634a71ad29b446a1e8236e0a45732ea5d0b4e86d9d1568ff5791412f093ec06f4f1', import.meta.url)
      new URL('./file/summarized_patients.csv', import.meta.url)
//      new URL('./file/patients.csv', import.meta.url)
    ]
  ])
//  console.log('fileAttachments keys', fileAttachments.keys())
//  console.log('fileAttachments values', fileAttachments.values())
//  console.log('fileAttachments entries', fileAttachments.entries())
//  console.log('fileAttachments size', fileAttachments.size)
  main.builtin('FileAttachment', runtime.fileAttachments(name => fileAttachments.get(name)))

//  d3.csv("./file/patients.csv", function(data_raw){
//    console.log(data_raw)
//  });
//  const fileAttachments_raw = new Map([
//    [
//      'patients_raw.csv',
//      new URL('./file/patients.csv', import.meta.url)
//    ]
//  ])
//  main.builtin('FileAttachment_raw', runtime.fileAttachments_raw(name => fileAttachments_raw.get(name)))
//  main.variable(observer('data_raw')).define('data_raw', ['FileAttachment_raw'], function (FileAttachment_raw) {
//    return (
//      FileAttachment('patients_raw.csv').csv({ typed: true })
//    )
//  })
  ...
  */
  main.variable(observer()).define(['md'], function (md) {
    return (
    md`# 北海道新型コロナウイルス感染者数の推移

新型コロナウイルス感染者数のグラフを表示します。 データ: [新型コロナウイルス感染症に関するデータ【北海道】](https://www.harp.lg.jp/opendata/dataset/)`
    )
  })
  ...
  */
  main.variable(observer('data')).define('data', ['FileAttachment'], function (FileAttachment) {
    return (
      FileAttachment('patients.csv').csv({ typed: true })
    )
  })
  ...
  */
  ...
  main.variable(observer('chart')).define(
    'chart',
    ['replay', 'd3', 'width', 'height', 'bars', 'axis', 'labels', 'ticker', 'keyframes', 'duration', 'x', 'invalidation'],
    async function*(replay, d3, width, height, bars, axis, labels, ticker, keyframes, duration, x, invalidation) {
      replay
  ...
  main.variable(observer('duration')).define('duration', function () { return (20) })
  main.variable(observer('n')).define('n', function () { return (12) })
  main.variable(observer('names')).define('names', ['data'], function (data) { return (new Set(data.map(d => d.name))) })
  ...
  main.variable(observer('keyframes')).define(
    'keyframes',
  ...
  main.variable(observer('bars')).define(
    'bars',
  ...
  main.variable(observer('labels')).define(
    'labels',
  ...
  main.variable(observer('ticker')).define('ticker', ['barSize', 'width', 'margin', 'n', 'formatDate', 'keyframes'],
  ...
  main.variable(observer('height')).define('height', ['margin', 'barSize', 'n'],
    function (margin, barSize, n) {
      return (margin.top + barSize * n + margin.bottom)
    }
  )
  main.variable(observer('barSize')).define('barSize', function () { return (48) })
  main.variable(observer('margin')).define('margin', function () { return ({ top: 16, right: 30, bottom: 6, left: 10 }) })
